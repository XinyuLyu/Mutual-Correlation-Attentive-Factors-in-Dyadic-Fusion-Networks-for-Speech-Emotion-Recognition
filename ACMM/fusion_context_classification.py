...
prediction = Dense(10,activation='softmax')(fusion_att_concat)
...
for i in range(size):
    ...
    loss_f, acc_f = model.evaluate([test_audio, test_text], test_label, batch_size=batch_size, verbose=0)
    output_test = model.predict([test_audio, test_text], batch_size=batch_size)
    acc_9 = compute_acc(output_test, test_label)
    acc_test.append(acc_f)
    loss_test.append(loss_f)
    print('epoch: ', str(i))
    print('loss_a', loss_f, ' ', 'acc_a', acc_f, 'acc', acc_9)
    if acc_f > acc:
        acc_9class = acc_9
        loss = loss_f
        acc = acc_f
        model.save_weights(save_path + 'fusion_context_classification.h5')
print('loss', loss, 'acc', acc, 'acc_9class',acc_9class)
